...
const baseImageName = "vecro-base:j13"
const mongoDBImageName = "vecro-mongodb:v2"
...
func prepareContainers(svc Service, sysName string) []apiv1.Container {
	...
	case "mongodb":
		...
		mongoDBExporterContainer := apiv1.Container{
			Name:  svc.Name + "-mongodb-exporter",
			Image: "percona/mongodb_exporter:0.20",
			Args: []string{
				"--mongodb.uri=mongodb://root:password@localhost:27017",
				"--discovering-mode",
				"--compatible-mode",
			},
			Ports: []apiv1.ContainerPort{
		...
		containers = append(containers, baseContainer, mongoDBContainer, mongoDBExporterContainer)
	}
