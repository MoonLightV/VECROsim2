...
func delay(delayTime int, delayJitter int) {
	...
	log.Printf("Info slept for %d miliseconds\n", delayTime + int(jitter))
}
...
func cpuStress(cpuLoad int) {
	...
	log.Printf("Info cpu load amount: %d, total sqrt(rand()): %d\n", cpuLoad, cpuOpsBase * cpuLoad)
}
...
func ioStress(ioLoad int) {
	...
	filename := "/tmp/ben-base-io/vecro_base_stress_" + strconv.FormatUint(rand.Uint64(), 10)
	...
	log.Printf("Info io load amount: %d, bytes to write: %d, total bytes written: %d\n", ioLoad, ioOpsBase * 8 * 128, totalBytes)
}
...
func allocMemory(size int) *[]int64 {
	...
	log.Printf("Info mem alloc: %d int64 summing up to %d MB is allocated.\n", size * memoryCountBase, size)
	return &object
}